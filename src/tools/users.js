...
export const usersTools = (zendeskClient) => [
  {
    name: "list_users",
    description: "List users in Zendesk. Returns maximum 20 users per page.",
    schema: {
      page: z.number().optional().describe("Page number for pagination"),
      role: z.enum(["end-user", "agent", "admin"]).optional().describe("Filter users by role")
    },
    handler: async ({ page, role }) => {
      try {
        const params = { page, per_page: 20, role };
