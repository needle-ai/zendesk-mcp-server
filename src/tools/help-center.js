...
export const helpCenterTools = (zendeskClient) => [
  {
    name: "list_articles",
    description: "List Help Center articles. Returns maximum 20 articles per page.",
    schema: {
      page: z.number().optional().describe("Page number for pagination"),
      sort_by: z.string().optional().describe("Field to sort by"),
      sort_order: z.enum(["asc", "desc"]).optional().describe("Sort order (asc or desc)")
    },
    handler: async ({ page, sort_by, sort_order }) => {
      try {
        const params = { page, per_page: 20, sort_by, sort_order };
